# services/brand_context.py
# Fetches brand guidelines from Google Drive and categorizes them
# into structured context fields for injection into the Claude prompt.
...
def categorize_file_content(filename: str, content: str, context: BrandContext) -> None:
    """Append file content to the appropriate BrandContext field based on filename keywords.

    Files are matched by substring in the filename (case-insensitive).
    A single file can match multiple categories.
    """
...
class BrandContextProvider(ABC):
    ...
    Implement this to add new sources: Notion, S3, local filesystem, etc.
    """

    @abstractmethod
    def get_brand_context(self) -> BrandContext:
        """Fetch and return brand context from this provider."""
        raise NotImplementedError


class GoogleDriveBrandContext(BrandContextProvider):
    """Fetches brand context from a Google Drive folder.

    Reads all files in the specified folder, categorizes them by filename,
    and returns a populated BrandContext ready for prompt injection.

    Requirements:
        - Service account JSON with Google Drive API read access
        - Service account email shared on the target Drive folder
        - GOOGLE_SERVICE_ACCOUNT_JSON env var pointing to the JSON path
    """
    ...
    def _list_files(self) -> list[dict]:
        """List all non-trashed files in the brand folder."""
    ...
    def get_brand_context(self) -> BrandContext:
        """Fetch all files from the Drive folder and return categorized BrandContext."""
        ...
        try:
            ...
            # Normalize: strip whitespace and fall back to placeholder
